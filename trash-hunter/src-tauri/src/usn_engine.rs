...
pub fn elite_scan_drive(app: &tauri::AppHandle, drive_letter: &str) -> Result<EliteIndexResult, String> {
    ...
    if count < MIN_FILES_EXPECTED {
        return Err(format!("Elite: Only found {} MFT items. Deploying Parachute.", count));
    }

    println!("ELITE: MFT Scan complete. Phase 2 (Reconstruction) on {} items...", name_map.len());
    ...
    for (id, name) in &name_map {
        ...
        if processed_count % 50_000 == 0 {
             println!("ELITE: Reconstructed {}/{} paths...", processed_count, name_map.len());
